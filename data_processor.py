...
def preprocess_video_sample(
    video_file_path, slice_duration_ms, mouth_height=128, mouth_width=128
):
    ...
    with VideoFileReader(video_file_path) as reader:
        ...
        mouth_cropped_frames = np.zeros(
            shape=(mouth_height, mouth_width, reader.get_frame_count()),
            dtype=np.float32,
        )
...
def preprocess_audio_signal(
    audio_signal, slice_duration_ms, n_video_slices, video_frame_rate
):
    ...
    mel_spectrogram, phase = signal_to_spectrogram(
        audio_signal, n_fft, hop_length, mel=True, db=True
    )
    ...
    slices = [
        mel_spectrogram[
            :,
            (i * spectrogram_samples_per_slice) : (
                (i + 1) * spectrogram_samples_per_slice
            ),
        ]
        for i in range(n_slices)
    ]
...
def reconstruct_speech_signal(mixed_signal, speech_spectrograms, video_frame_rate):
    ...
    _, original_phase = signal_to_spectrogram(
        mixed_signal, n_fft, hop_length, mel=True, db=True
    )
...
def signal_to_spectrogram(audio_signal, n_fft, hop_length, mel=True, db=True):
    ...
    if mel:
        mel_filterbank = librosa.filters.mel(
            sr=audio_signal.get_sample_rate(), n_fft=n_fft, n_mels=80, fmin=0, fmax=8000
        )
...
def preprocess_audio_pair(
    ...
    mixed_spectrograms = preprocess_audio_signal(
        mixed_signal, slice_duration_ms, n_video_slices, video_frame_rate
    )
    speech_spectrograms = preprocess_audio_signal(
        speech_signal, slice_duration_ms, n_video_slices, video_frame_rate
    )
    noise_spectrograms = preprocess_audio_signal(
        noise_signal, slice_duration_ms, n_video_slices, video_frame_rate
    )
...
def preprocess_sample(speech_entry, noise_file_path, slice_duration_ms=200, snr_db=0):
    print(
        "preprocessing sample: %s, %s, %s, %s..."
        % (speech_entry.video_path, speech_entry.audio_path, noise_file_path, snr_db)
    )

    video_samples, video_frame_rate = preprocess_video_sample(
        speech_entry.video_path, slice_duration_ms
    )
    (
        mixed_spectrograms,
        speech_spectrograms,
...
def try_preprocess_sample(preprocessing_arguments):
    try:
        return preprocess_sample(**preprocessing_arguments)

    except Exception as e:
        print("failed to preprocess %s (%s)" % (preprocessing_arguments, e))
        return None


def preprocess_data(speech_entries, noise_file_paths, snr_db=0):
    ...
    preprocessing_arguments = [
        ...
        for s, n in zip(speech_entries, noise_file_paths)
    ]

    thread_pool = multiprocessing.Pool(8)
    samples = thread_pool.map(try_preprocess_sample, preprocessing_arguments)
