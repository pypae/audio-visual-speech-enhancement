...
def pesq(pesq_bin_path, source_file_path, enhanced_file_path):
    ...
    match = re.search(
        "\(Raw MOS, MOS-LQO\):\s+= (-?[0-9.]+?)\s+([0-9.]+?)$", output, re.MULTILINE
    )
...
def evaluate(enhancement_dir_path, pesq_bin_path):
    ...
    for speaker_dir_name in speaker_dir_names:
        ...
        for sample_dir_name in sample_dir_names:
            try:
                ...
                enhanced_mos, _ = pesq(
                    pesq_bin_path, source_file_path, enhanced_file_path
                )
                mixture_mos, _ = pesq(
                    pesq_bin_path, source_file_path, mixture_file_path
                )

                print (
                    "mixture pesq: %f, enhanced pesq: %f" % (mixture_mos, enhanced_mos)
                )
    ...
    print "mean enhanced pesq: ", np.mean(
        enhanced_pesqs
    ), "std enhanced pesq: ", np.std(enhanced_pesqs)
    print "mean mixture pesq: ", np.mean(mixture_pesqs), "std mixture pesq: ", np.std(
        mixture_pesqs
    )
