...
def preprocess(args):
    ...
    speech_entries, noise_file_paths = list_data(
        ...
        oversample_noise=args.oversample_noise,
    )

    samples = data_processor.preprocess_data(
        speech_entries, noise_file_paths, snr_db=args.snr_db
    )

    with open(
        assets.get_preprocessed_blob_path(args.data_name), "wb"
    ) as preprocessed_fd:
...
class PredictionStorage(object):
    def __init__(self, storage_dir):
        self.__base_dir = os.path.join(
            storage_dir, "{:%Y-%m-%d_%H-%M-%S}".format(datetime.now())
        )
    ...
    def save_prediction(self, sample, predicted_speech_signal):
        ...
        sample_prediction_dir = os.path.join(
            speaker_dir, speech_name + "_" + noise_name
        )
        ...
        mixture_video_path = os.path.join(
            sample_prediction_dir, "mixture" + video_extension
        )
        enhanced_speech_video_path = os.path.join(
            sample_prediction_dir, "enhanced" + video_extension
        )

        ffmpeg.merge(sample.video_file_path, mixture_audio_path, mixture_video_path)
        ffmpeg.merge(
            sample.video_file_path,
            enhanced_speech_audio_path,
            enhanced_speech_video_path,
        )
...
def make_sample_set(samples, max_samples=None):
    ...
    mixed_spectrograms = np.concatenate(
        [sample.mixed_spectrograms for sample in samples], axis=0
    )
    speech_spectrograms = np.concatenate(
        [sample.speech_spectrograms for sample in samples], axis=0
    )
    ...
    return (video_samples, mixed_spectrograms, speech_spectrograms)
...
def main():
    ...
    preprocess_parser.add_argument(
        "-n", "--noise_dirs", nargs="+", type=str, required=True
    )
    ...
    preprocess_parser.add_argument("-snr", "--snr_db", type=int, default=0)
    preprocess_parser.add_argument("--max_files", type=int, default=None)
    preprocess_parser.add_argument("--augmentation_factor", type=int, default=3)
    preprocess_parser.add_argument("--oversample_noise", type=bool, default=True)
    preprocess_parser.set_defaults(func=preprocess)

    train_parser = action_parsers.add_parser("train")
    train_parser.add_argument("-mn", "--model", type=str, required=True)
    train_parser.add_argument(
        "-tdn", "--train_data_names", nargs="+", type=str, required=True
    )
    train_parser.add_argument(
        "-vdn", "--validation_data_names", nargs="+", type=str, required=True
    )
