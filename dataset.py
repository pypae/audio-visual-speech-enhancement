...
AudioVisualEntry = namedtuple(
    "AudioVisualEntry", ["speaker_id", "audio_path", "video_path"]
)
    ...
    def subset(self, speaker_ids, max_files=None, shuffle=False):
        ...
        for speaker_id in speaker_ids:
            audio_paths = glob.glob(
                os.path.join(self._base_path, speaker_id, "audio", "*.wav")
            )
...
class AudioDataset:
    ...
    def subset(self, max_files=None, shuffle=False):
        audio_file_paths = [
            os.path.join(d, f) for d in self._base_paths for f in os.listdir(d)
        ]
