...
class SpeechEnhancementNetwork(object):
    ...
    def __build_audio_encoder(audio_input):
        x = Convolution2D(64, kernel_size=(5, 5), strides=(2, 2), padding="same")(
            audio_input
        )
        x = BatchNormalization()(x)
        x = LeakyReLU()(x)
    ...
    def __build_audio_decoder(embedding):
        x = Deconvolution2D(128, kernel_size=(2, 2), strides=(2, 1), padding="same")(
            embedding
        )
        x = BatchNormalization()(x)
        x = LeakyReLU()(x)
    ...
    def train(
        ...
        train_mixed_spectrograms = np.expand_dims(
            train_mixed_spectrograms, -1
        )  # append channels axis
        train_speech_spectrograms = np.expand_dims(
            train_speech_spectrograms, -1
        )  # append channels axis

        validation_mixed_spectrograms = np.expand_dims(
            validation_mixed_spectrograms, -1
        )  # append channels axis
        validation_speech_spectrograms = np.expand_dims(
            validation_speech_spectrograms, -1
        )  # append channels axis

        checkpoint = ModelCheckpoint(model_cache_path, verbose=1)

        lr_decay = ReduceLROnPlateau(
            monitor="val_loss", factor=0.5, patience=5, min_lr=0, verbose=1
        )
    ...
    def predict(self, mixed_spectrograms, video_samples):
        mixed_spectrograms = np.expand_dims(
            mixed_spectrograms, -1
        )  # append channels axis
    ...
    def evaluate(self, mixed_spectrograms, video_samples, speech_spectrograms):
        ...
        speech_spectrograms = np.expand_dims(
            speech_spectrograms, -1
        )  # append channels axis

        loss = self.__model.evaluate(
            x=[mixed_spectrograms, video_samples], y=speech_spectrograms
        )
